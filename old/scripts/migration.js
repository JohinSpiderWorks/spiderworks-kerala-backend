...
const pageMigration = async () => {
  try {
    // Step 1: Add 'slug' as NULLABLE
    await queryInterface.addColumn('pages', 'slug', {
      type: Sequelize.STRING,
      allowNull: true, // make nullable temporarily
    });
    console.log("Step 1: 'slug' column added (nullable)");

    // Step 2: Update existing records with some slug (example: 'page-<id>')
    await queryInterface.sequelize.query(`
      UPDATE pages 
      SET slug = CONCAT('page-', id)
      WHERE slug IS NULL
    `);
    console.log("Step 2: Updated existing 'pages' rows with default slug");
    ...
    `);
    console.log("Step 3: Set 'slug' column to NOT NULL");
    ...
    console.log("Step 4: Added UNIQUE constraint to 'slug'");

    // Step 5: Add 'og_title' column
    await queryInterface.addColumn('pages', 'og_title', {
      type: Sequelize.STRING,
      allowNull: true,
    });
    console.log("Added 'og_title' column to 'pages'.");

    // Step 6: Add 'og_description' column
    await queryInterface.addColumn('pages', 'og_description', {
      type: Sequelize.TEXT,
      allowNull: true,
    });
    console.log("Added 'og_description' column to 'pages'.");

    // Step 7: Add 'og_image' column
    await queryInterface.addColumn('pages', 'og_image', {
      type: Sequelize.STRING,
      allowNull: true,
    });
    console.log("Added 'og_image' column to 'pages'.");

    console.log("✅ Page migration completed successfully.");
  } catch (error) {
    console.error("❌ Error during page migration:", error);
  } finally {
