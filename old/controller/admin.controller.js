...
//comment because removing the opt verfication
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (!checkPassword) {
      const currentAttempts = Number(findAdmin.dataValues.failed_login_attempts) || 0;
      const newFailedAttempts = currentAttempts + 1;
    ...
    await userModel.update(
      ...
      { where: { id: findAdmin.dataValues.id }, transaction }
    );

    // Generate token
    const token = jwt.sign(
      findAdmin.dataValues.id.toString(),
      process.env.JWT_SECRET_TOKEN
    );
...
const updateBlogAttributes = async (req, res) => {
  try {
    ...
    const allowedTypes = ["banner", "featured", "standard", "none"];
    ...
    if (typeof premium !== 'undefined') blog.premium = premium;
    if (typeof is_published !== 'undefined') blog.is_published = is_published;
